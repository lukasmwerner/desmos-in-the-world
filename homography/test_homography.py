...
while cv2.waitKey(1) != ord('q'):
    ...
    for marker_corner_list in marker_corners:
        points = cv2.perspectiveTransform(marker_corner_list.reshape(-1, 1, 2), camera_to_monitor)
        monitor_frame = cv2.fillPoly(monitor_frame, [points.astype(np.uint8)], (0, 255, 255))
